...
function useScroll(pagesIds) {
  ...
  const firstLightColor = getComputedStyle(
    document.documentElement
  ).getPropertyValue("--first-color-light");
  const secondDarkColor = getComputedStyle(
    document.documentElement
  ).getPropertyValue("--second-color-dark");
  const thirdColor = getComputedStyle(
    document.documentElement
  ...
  function scrolling() {
    ...
    const activePage = pagesElements.find((page, index, elements) => {
      ...
      if (isMobile) {
        ...
        contactPageActive =
          previousElt &&
          previousElt.className === "about-me" &&
          previousElt.getBoundingClientRect().bottom <
            window.innerHeight - offset;
        aboutMePageNonActive =
      ...
      return (
        ...
        (!aboutMePageNonActive &&
          ...
          elementRect.top > -window.innerHeight - offset &&
  ...
  useEffect(() => {
    ...
    document
      .querySelector("#vertical-snapping")
      .addEventListener("scroll", scrolling);
    document
      .querySelector("#vertical-snapping")
      .addEventListener("touchmove", scrolling);
